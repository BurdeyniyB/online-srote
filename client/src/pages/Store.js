...
const Store = observer(() => {
  ...
  useEffect(() => {
    ...
    fetchDevices(null, null, 1, device.limit, null).then((data) => {
  ...
  useEffect(() => {
    console.log(
      "useEffect" +
        "\nselected type: " + JSON.stringify(device.selectedType) +
        "\nselected brand: " + JSON.stringify(device.selectedBrand) +
        "\nselected sort:" + JSON.stringify(device.selectedSortBy.id)
    );
  ...
  }, [device.selectedType, device.selectedBrand, device.minPrice, device.maxPrice, device.selectedSortBy, device.page, device.search]);
