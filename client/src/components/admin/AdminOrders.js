...
const orderBadge = (s) => {
  ...
  return (
    <span className={`status-badge ${map[s] || "status-new"}`}>
      {s || "new"}
    </span>
  );
};
const payBadge = (s) => {
  ...
  return (
    <span className={`status-badge ${map[s] || "pay-unpaid"}`}>
      {s || "unpaid"}
    </span>
  );
};
const delBadge = (s) => {
  const map = {
    ...
    returned: "del-returned",
  };
  return (
    <span className={`status-badge ${map[s] || "del-pending"}`}>
      {s || "pending"}
    </span>
  );
};
      ...
      <div className="orders-filters-bar">
        ...
        <div className="filter-group">
          ...
          <select
            ...
            onChange={(e) =>
              handleFilterChange("delivery_status", e.target.value)
            }
          >
            {DEL_STATUSES.map((s) => (
              <option key={s} value={s}>
          ...
          <table className="orders-table">
            ...
            <tbody>
              {orders.map((o) => (
                ...
                >
                  <td className="order-id-cell">#{o.order_number}</td>
